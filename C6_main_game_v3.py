...
class Play:
    def __init__(self, how_many, partner):
        ...
        self.rounds_heading = Label(self.play_frame, text=f"Round {self.round} of {self.how_many}",
        ...
        self.feedback_label = Label(self.play_frame, text="", font=button_font, bg=button_bg, fg=button_fg,
                                    wraplength=300, justify="center")
        self.feedback_label.grid(row=2, columnspan=4, pady=10)  # Added feedback label
        ...
        self.next_round_button.set_state("disabled")  # Initially disabled
    ...
    def display_question(self):
        ...
        self.feedback_label.config(text="")  # Clear feedback label for the new question

        for i, button in enumerate(self.option_buttons):
            button.update_text(options[i])
            button.set_state("normal")  # Enable option buttons for a new question

        self.next_round_button.set_state("disabled")  # Disable "Next Round" button

    # checks the selected answer is correct and update the score
    def check_answer(self, selected_index):
        if selected_index == self.correct_index:
            self.score += 1
            feedback = f"Correct! It is {self.all_fears[0].split(' - ')[1]}"
            self.option_buttons[selected_index].itemconfig(self.option_buttons[selected_index].button_bg, fill="green")
        else:
            feedback = f"Wrong! The correct answer is {self.all_fears[0].split(' - ')[1]}"
            self.option_buttons[selected_index].itemconfig(self.option_buttons[selected_index].button_bg, fill="red")
            self.option_buttons[self.correct_index].itemconfig(self.option_buttons[self.correct_index].button_bg, fill="green")

        self.feedback_label.config(text=feedback)  # Display feedback message

        # Disable option buttons after selection
        for button in self.option_buttons:
            button.set_state("disabled")

        # Enable the "Next Round" button to proceed
    ...
    def end_game(self):
        ...
        self.feedback_label.config(text="")  # Clear feedback label at game end
        for button in self.option_buttons:
            button.set_state("disabled")
